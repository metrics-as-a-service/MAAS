const HELP_TEXT = [
    ...
    },
    {
        ...
        details: [
            ...
            "<b>2x2</b> shows the cross tabulated data for two columns. You can select the x and y axes.  There is an option of 'average/sum' and 'count if' as described in the next section.",,
            ...
            "<b>List Members</b> is useful when a column contains a list of values as described above. The chart shows the count of tags in the input. You can further define “count if“ value as described in the next section.", 
            ...
            "<b>Plan</b> shows the data in a Gantt chart The inputs required are column that has the description to show in the plan, cowman for start date and column for the end date. Columns for start and end dates must contain dates. You can further define “count if“ as described in the next section.",
            "<b>Risk</b> shows the data in standard 5x5 risk matrix. You can select the likelihood and impact columns. The App will expect the values 1 to 5 in these columns. You can define the likelihood and impact values as well. There are options of 'average/sum' and 'count if' as described in the next section.",
            "<b>State Change</b> is it a chart that shows data on transitions. It expects that the data source is sorted in acceding order for each item. You have to select the columns from where the App picks the values: Time stamp shows the time of the transition while From and To give the values at the point of transition. There is an option to count the number of transition or calculate the total time of transition or average time for transition.  There is also an option of 'count if' as described in the next section.",
            "<b>String</b> is a is used when the x-axis is neither Date or Number. You can define an “order” that will make sure (a) the x-axis will be ordered accordingly (b) the item will be shown on the chart even if the value is zero. For instance, you want to show P1 even if zero then enter P1 in “order”.  There are options of 'average/sum' and 'count if' as described in the next section.",
    ...
    },
    {
        ...
        details: [
            "<b>Average/sum</b> where the App uses another column, called 'column over', that contains numeric data.  The App displays the sum or average of the values form this second column.",
