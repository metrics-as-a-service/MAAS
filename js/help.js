const HELP_TEXT = [
    ...
    },
    {
        ...
        details: [
            ...
            "<b>Modify Layout</b> lets you update the chart title of the “data as of date”. This is the”report date” which is used in other charts (W4, Trend and Trend OC),",
            "<b>Dowload Main Config</b> allows you to download the current config. You can use the file later. For some reason the download is not workling consistantly in all browser.  It download is unsucessful the cofig is copied to clipboard and the workaround is to open a new text file, paste the config and save the file",
    ...
    },
    {
        ...
        details: [
            ...
            "<b>Auto title</b> indicates if the title should be automatically set by the App. It is recommended that you keep this checked while you are configuring the charts. Once you are done, you can uncheck this otion and enter your value.",
            ...
            "The other values are dependent on the chart type as desribed in the next section.",
        ]
    },
    {
        header: "How to configue charts using chart types?",
        details: [
            "When configuring a chart different options, relevant to the chart type, will be available as decribed below:",
            "<b>2x2</b> shows the cross tabulated data for two columns. You can select the x and y axex.  There is an option of 'average/sum' and 'count if' as described in the next section.",,
            ...
            "<b>Date</b> is used when the x-axis has date values. Choose the data table chart type date means the x-xis state at the column column ID.You can select the column to show in the x-axis. Different formats available such as DD (day of the month), DDD (day of the week), MMM (month), MMM-YY (pecific month) and Wx (weeks from report date). There is an option of 'average/sum' and 'count if' as described in the next section.",
            "<b>List Count</b> is useful when a column contains a list of values, for example list of tags or list of linked defects to a test script.You have to indicate a separator for the list, such as comma. The separator defaults to a space if not provided.  There is an option 'count if' as described in the next section.",
            "<b>List Members</b> is useful when a column contains a list of values as described above. The chart shows the count of tags in the input. You can further define “count if“ value as described in the next section.", //TO DO
            ...
            "<b>Number</b> is used when the x-axis has numbers. There is an option for youth enter “bin” to show chart by bin values. The bin if entered must be a comma separated list of numbers in ascending order.  You can further define “count if“ as described in the next section.",
            "<b>Plan</b> shows the data in a Gantt chart The inputs required are column that has the description to show in the plan, cowman for start date and column for the end date.Clearly columns for start and end dates must contain dates. You can further define “count if“ as described in the next section.",
            "<b>Risk</b> shows the data in standard 5x5 risk matrix.You can select the likelihood and impact columns. The App will expect the values 1 to 5 in these columns.You can define define the likelihood and impact values as well.  There is an option of 'average/sum' and 'count if' as described in the next section.",
            "<b>State Change</b> is it a chart that shows data on transitions. It expects that the data source is sorted in acceding oder for each item. You have to select thee columns from where the App picks the values: Time stamp shows the time of the transition while From and To give the values at the point of transition. There is an option to count the number of transition or calculate the total time of transition or average time for transition.  There is an option of 'count if' as described in the next section.",
            "<b>String</b> is a is used when the x-axis is neither Date or Number. You can define an “order” that will make sure (a) the x-axis will be ordered accordingly (b) the item will be shown on the chart even if the value is zero. For instance, you want to show P1 even if zero then enter P1 in “order”.  There is an option of 'average/sum' and 'count if' as described in the next section.",
            "<b>Trend</b> shows trend of values; for example test execution trend. You have to specify a date column. You can further define “count if“, “plan“ and “forecast“ values as described in the next section.",
            "<b>Trend OC</b> is very similar to Trend except it calculated the trend using ‘open’ and ‘close’ dates. You can further define “count if“, “plan“ and “forecast“ values as described in the next section.",
        ]
    },
    {
        header: "How to set average/sum, countif, plan and forecast?",
        details: [
            "<b>Average/sum</b> whre the App uses another column, called 'column over', that contains numeric data.  The App displays the sum or average of the values form this second column.",
            "Follwing parametres are set using string as an input. To aid your input, a template is displayed when the input field is first clicked. The templates are explained below. Where _x_ is shown in the template a value has to be provided.",
            ...
            "<b>Forecast</b> has two templates.  For Chart Type as Trend the tempalte is <mark>“basisdays  _number_  forecasdays  _number_  changefrom  _date_  count  _number_“.</mark> The basisdays indiacte the number of days previous to the report date used to calculate forecast, forecasdays indiactes the numer of days forecast is displayed, changefrom is a date where the forecast is modifed the value for count.", //TO DO
            "<b>Forecast for Trend OC</b> has the tempalte is <mark>“basisdays  _number_  forecasdays  _number_  changefrom  _date_  open  _number_  close  _number_“.</mark> The meaning of most paramters are as above excep open and close that indiacte how the forecast is modifed.",
            "",
        ]
    },
]
