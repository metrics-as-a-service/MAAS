...
const $c = (function () {
    ...
    //  no              yes     any     pass 1 and pass 2 (done)
    ...
    self.processCSVFile = async function (inputJSON, localFile) {
        ...
        function countRecords(allCounts, row) {
            ...
            function countPlan(key, oneCount, isFiltered) {
                ...
                const {
                    ...
                    secondEndDateCol,
                    RAGCol,
                } = getChartProps(key) //$p.getChartProps(key)
            ...
            function countTrend(key, oneCount, isFiltered) {
                ...
                function initForecast(forecast, memo) {
                    ...
                    if (!allCounts.memo[key]) allCounts.memo[key] = {}
                    const { reportDate } = config
                    const cutoffDate = addDays(reportDate, -(lookBack - 1))
                ...
                function updateCounter() {
                    ...
                    function updateForecast({
                        ...
                        const dateDiff = dateTimeDiff(cutoffDate, date) - 1
                        if (dateDiff >= 0) {
            ...
            function countStateChange(key, oneCount, filtered) {
                ...
                function readyPrevious({
                    ...
                    const prev = oneCount[SYMBOL_PREV_STATE]
                    // oneCount[SYMBOL_PREV_STATE] = { id, to, from, timestamp }
        ...
        function cleanRow(row, presetOffsetDays) {
            ...
            for (const key in row) {
                ...
                if (_isValidDate(cell)) cleanedRow[key] = getModifiedDate(cell)
                else cleanedRow[key] = cell
            }
            return cleanedRow
        }
    ...
    const dataTypes = {
        Date: {
            ...
            getFormattedValue: (v, { dateFormat = "MMM", reportDate }) => {
                ...
                if (positionOfW === -1) return formatDate(v, dateFormat)
                ...
                const days = dateTimeDiff(reportDate, v, "Days")
    ...
    const chartTypes = {
        ...
        "Trend OC": {
            ...
            orderedValues: (chartProp, trendEndDate) => {
                ...
                const datePoints = []
                const dateDifference = dateTimeDiff(
                    trendStartDate,
                    trendEndDate,
                    "Days"
                ) //+ 1
    ...
    self.validate = (chartType, properties, config) => {
        const error = {
            isValid: false,
    ...
    self.validateCallout = (chartType, properties, config) => {
        const error = {
            isValid: false,
            errors: {
            ...
            if (chartType == "Trend OC" || chartType == "Trend") {
                ...
                function addForecast(plotData) {
                    ...
                    const endPoint = () => {
                        const endDate =
                            forecastTo === "max"
                                ? addDays(cutoffDate, endXModified)
                                : _isValidDate(forecastTo)
                                ? forecastTo
                                : addDays(cutoffDate, forecastTo)

                        const endCount =
                            forecastTo === "max"
                                ? filteredRowCounts
                                : getY(
                                      dateTimeDiff(cutoffDate, endDate),
                                      linear
                                  )
